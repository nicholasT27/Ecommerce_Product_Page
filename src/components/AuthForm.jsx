...
export default function AuthForm() {
    ...
    return <div className="w-full max-w-md mx-auto bg-lgblue rounded-2xl p-6 lg:p-8">
        <div className="grid grid-cols-2 bg-white rounded-xl p-1 mb-6">
            <button type="button" onClick={() => setMode('signin')} className={`rounded-lg py-2 font-bold ${mode === 'signin' ? 'bg-vdblue text-white' : ''}`}>Sign in</button>
            <button type="button" onClick={() => setMode('signup')} className={`rounded-lg py-2 font-bold ${mode === 'signup' ? 'bg-vdblue text-white' : ''}`}>Create account</button>
        </div>
